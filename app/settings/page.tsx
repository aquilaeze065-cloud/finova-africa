...
export default function SettingsPage() {
  ...
  return (
    <MobileLayout activePage="Settings">
      <style>{`
        .st-tabs{display:flex;background:rgba(0,200,150,0.05);border:1px solid rgba(0,200,150,0.12);border-radius:14px;padding:0.25rem;gap:0.25rem;margin-bottom:1.3rem;}
        .st-tab{flex:1;padding:0.55rem 0.3rem;border-radius:10px;border:none;cursor:pointer;font-size:0.76rem;font-weight:600;transition:all 0.2s;background:none;color:#5a8a7a;display:flex;align-items:center;justify-content:center;gap:0.3rem;}
        .st-tab.active{background:linear-gradient(135deg,#00a87a,#00c896);color:#050f0c;font-weight:700;}
        .st-label{font-size:0.7rem;color:#5a8a7a;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:0.4rem;display:block;}
        .st-input{width:100%;background:rgba(0,200,150,0.05);border:1px solid rgba(0,200,150,0.15);border-radius:11px;padding:0.75rem 0.9rem;font-size:0.9rem;color:#e8f8f4;font-family:"DM Sans",sans-serif;outline:none;transition:border-color 0.2s;margin-bottom:0.85rem;}
        .st-input:focus{border-color:rgba(0,200,150,0.35);}
        .st-btn{width:100%;padding:0.9rem;border:none;border-radius:13px;background:linear-gradient(135deg,#00a87a,#00c896);font-family:"Inter",serif;font-weight:700;font-size:1rem;color:#050f0c;cursor:pointer;box-shadow:0 0 20px rgba(0,200,150,0.2);transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:0.5rem;margin-top:0.5rem;}
        .st-btn:hover{transform:translateY(-2px);}
        .st-btn:disabled{opacity:0.5;cursor:not-allowed;transform:none;}
        .st-err{background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.25);border-radius:9px;padding:0.6rem 0.8rem;font-size:0.8rem;color:#ff4757;margin-bottom:0.8rem;}
        .st-toggle-row{display:flex;align-items:center;justify-content:space-between;padding:0.9rem;background:rgba(0,200,150,0.03);border:1px solid rgba(0,200,150,0.08);border-radius:12px;margin-bottom:0.6rem;cursor:pointer;transition:all 0.18s;}
        .st-toggle-row:hover{border-color:rgba(0,200,150,0.2);}
        .st-toggle-info{flex:1;}
        .st-toggle-title{font-weight:600;font-size:0.88rem;margin-bottom:0.15rem;}
        .st-toggle-sub{font-size:0.72rem;color:#5a8a7a;}
        ...
        .st-toggle.on{background:linear-gradient(135deg,#00a87a,#00c896);}
        ...
        .st-photo-av{width:100px;height:100px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:700;color:#050f0c;border:3px solid rgba(0,200,150,0.4);overflow:hidden;cursor:pointer;position:relative;box-shadow:0 0 24px rgba(0,200,150,0.25);}
      ...
      <div style={{fontFamily:"Inter,serif",fontWeight:800,fontSize:"1.3rem",marginBottom:"0.3rem"}}>⚙️ Settings</div>
      <div style={{fontSize:"0.82rem",color:"#5a8a7a",marginBottom:"1.2rem"}}>Manage your account preferences</div>
      ...
      {tab==="profile"&&(
        <div className="r-card">
          <div style={{fontFamily:"Inter,serif",fontWeight:700,fontSize:"0.95rem",color:"#00c896",marginBottom:"1rem"}}>📸 Profile Photo</div>
          ...
          <div className="st-photo-wrap">
            <div className="st-photo-av" onClick={()=>photoRef.current?.click()}
              style={{background:photoPreview?"transparent":"linear-gradient(135deg,#00a87a,#00c896)"}}>
              {photoPreview
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontFamily:"Inter,serif",fontWeight:700,fontSize:"1rem",marginBottom:"0.2rem"}}>{user?.name}</div>
              <div style={{fontSize:"0.78rem",color:"#5a8a7a"}}>{user?.email}</div>
              <div style={{fontSize:"0.72rem",color:"#5a8a7a",marginTop:"0.2rem"}}>ID: {user?.userId}</div>
            </div>
            <button onClick={()=>photoRef.current?.click()} style={{padding:"0.6rem 1.5rem",background:"rgba(0,200,150,0.1)",border:"1px solid rgba(0,200,150,0.25)",borderRadius:"10px",color:"#00c896",cursor:"pointer",fontFamily:"Inter,serif",fontWeight:700,fontSize:"0.85rem"}}>
              Choose New Photo
            </button>
          </div>
          {photo&&(
          ...
          <div style={{borderTop:"1px solid rgba(0,200,150,0.1)",paddingTop:"1rem",marginTop:"1rem"}}>
            <div style={{fontFamily:"Inter,serif",fontWeight:700,fontSize:"0.9rem",color:"#00c896",marginBottom:"0.75rem"}}>Account Info</div>
            {[
              ...
              <div key={r.l} style={{display:"flex",justifyContent:"space-between",padding:"0.5rem 0",borderBottom:"1px solid rgba(0,200,150,0.06)",fontSize:"0.84rem"}}>
                ...
                <span style={{fontWeight:600,color:"#e8f8f4",textAlign:"right",maxWidth:"60%",fontSize:"0.82rem"}}>{r.v}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab==="password"&&(
        <div className="r-card">
          <div style={{fontFamily:"Inter,serif",fontWeight:700,fontSize:"0.95rem",color:"#00c896",marginBottom:"1rem"}}>🔒 Change Password</div>
          ...
          <div style={{background:"rgba(0,200,150,0.06)",border:"1px solid rgba(0,200,150,0.12)",borderRadius:"10px",padding:"0.75rem",fontSize:"0.78rem",color:"#5a8a7a",lineHeight:1.5,marginBottom:"0.5rem"}}>
      ...
      {tab==="notifs"&&(
        <div className="r-card">
          <div style={{fontFamily:"Inter,serif",fontWeight:700,fontSize:"0.95rem",color:"#00c896",marginBottom:"1rem"}}>🔔 Notification Preferences</div>
          {[
            ...
            {key:"adminMessages",   title:"Admin Messages",             sub:"Important messages from NEXORA team"},
          ].map((n: any) =>(
